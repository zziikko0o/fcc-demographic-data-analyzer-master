...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = round((df[df.education == "Bachelors"].count()['education'] / df.shape[0]) * 100, 1)
    ...
    num_min_workers = df.query('salary == ">50K" and `hours-per-week` == @min_work_hours').shape[0]
    ...
    country_stats = df[df.salary == '>50K'].groupby('native-country')['salary'].count() / df.groupby('native-country')[
        'salary'].count()
